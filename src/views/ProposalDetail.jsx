...
const ProposalDetail = ({ proposals, events, currentUser }) => {
  ...
  const proposal = proposals.find(p => String(p.proposal_id || p.id) === String(id));

  const handleUpdateStatus = async (newStatus) => {
    setIsSyncing(true);
    const updatedProposal = { ...proposal, status: newStatus };
    await cloudSync.push('proposals', proposal.proposal_id || proposal.id, updatedProposal);
  ...
  const handleDelete = async () => {
    ...
    setIsSyncing(true);
    await cloudSync.push('proposals', proposal.proposal_id || proposal.id, null);
  ...
  if (!proposal) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-400">
        <h2 className="text-2xl font-bold mb-4">Proposal tidak ditemukan</h2>
        <button onClick={() => navigate(-1)} className="text-[#0ea5e9] font-bold">Kembali</button>
      </div>
    );
  }
        ...
        <div className="space-y-12">
           <section>
              <h1 className="text-4xl font-black font-['Outfit'] mb-6 tracking-tight leading-tight text-[#0f172a]">
                {proposal.title_proposal || proposal.title}
              </h1>
              <div className="flex flex-wrap gap-4">
                 <InfoChip icon={<Clock size={14} />} label="Submitted On" value={proposal.submission_date || proposal.date || 'N/A'} />
                 <InfoChip icon={<FileText size={14} />} label="Event ID" value={proposal.event_id || proposal.programId || 'N/A'} />
